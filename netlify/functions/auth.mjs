...
import { verifyGoogleToken, bearer, isAdminEmail } from "../lib/auth.mjs";
...
export default async (req) => {
  ...
  if (!user) {
    return Response.json({ signedIn: false, isAdmin: false }, { headers: HEADERS });
  }
  const isAdmin = await isAdminEmail(user.email);
  return Response.json(
    { signedIn: true, isAdmin, email: user.email, name: user.name, picture: user.picture },
    { headers: HEADERS },
  );
};
